...
export function AnalysisLanding({
  ...
  const { analysisQuery } = useLandingAnalysisFromCache()
  ...
  const videoIdRender = analysis.data?.videoId ?? analysisQuery.data?.videoId ?? 'ucX2zXAZ1I0'
  const analysisCache = analysisQuery.data

  return (
    <>
      <Card fullWidth className="p-4 h-[40rem] bg-gray-100">
        {analysis.isLoading || analysisQuery.isLoading ? (
